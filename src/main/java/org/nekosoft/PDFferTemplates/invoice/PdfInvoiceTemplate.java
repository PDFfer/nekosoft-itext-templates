...
/**
 * A template for PDFfer that takes an {@link InvoiceData} instance as payload and
 * generates a PDF invoice document from it.
 */
@PdfTemplateComponent(group = "nekosoft", name = "invoice")
public class PdfInvoiceTemplate extends AbstractJacksonPdfTemplate<InvoiceData> {
    ...
    public PdfInvoiceTemplate() {
            getJsonMapper().registerModule(new JavaTimeModule());
    }
    ...
    protected InvoiceData initPayload(InvoiceData data) {
        try {
            ...
            priceFormat.setCurrency(Currency.getInstance(data.getCurrency()));
            return data;
    ...
    public void generate() {
        InvoiceData invoiceData = getPayload();
    ...
    private void addLineItems() {
        ...
        for (LineItemData item : getPayload().getLineItems()) {
    ...
    private void addLineItemSummary() {
        ...
        for (Map.Entry<String, Double> tax : getPayload().getTaxSubtotals().entrySet().stream().sorted(Comparator.comparing(Map.Entry::getKey)).collect(Collectors.toList())) {
